  ...
  return (
    <div className="w-full max-w-md border rounded-lg shadow bg-gray-800 border-gray-700 h-96 flex flex-col no-select no-drag">
      ...
      <div className="py-5 flex flex-row justify-end">
        <Link to={`/product/id:${product.id}`} onClick={() => {getProduct(product.id);}} className="text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm mb-2 mx-5 py-2 text-center bg-blue-600 hover:bg-blue-700 focus:ring-blue-800 w-1/3 no-select no-drag"> Buy </Link>
      </div>
    </div>
  );
};
